...
export default function AddClientPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      toast({
        title: t("clients", "clientAdded", language),
        description: t("clients", "clientAddedSuccess", language, { name: formData.name }),
      })
      setIsSubmitting(false)
      router.push("/clients")
    }, 1000)
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between bg-white p-4 rounded-lg shadow-sm border">
          <div className="space-y-1">
            <h2 className="text-2xl font-bold tracking-tight">{t("clients", "createClient", language)}</h2>
            <p className="text-muted-foreground text-sm">{t("clients", "createClientDescription", language)}</p>
          </div>
